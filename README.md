...
- Median of 3 runs; reproduce with `mvn test -Pbenchmark`. A run taken while the machine had heavy background load produced numbers up to 75 % lower
...
Coverage: **77 % instruction / 52 % branch** (JaCoCo gate: 40 % instruction, enforced in `mvn verify`).
